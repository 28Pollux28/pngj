...
public class ImageLineHelper {
			...
			if (alphachannel) // same format!
				...
				for (int c = 0, b = 0; b < bytes;) {
					...
					if (trns != null && buf[b - 3] == (byte) trns.getRGB()[0] && buf[b - 2] == (byte) trns.getRGB()[1]
							&& buf[b - 1] == (byte) trns.getRGB()[2]) // not
						// very
						// efficient,
						// but
						// not
	 ...
	 * 
	 * This only makes sense for ARGB images, with bitdepth=8 (this is not
	 * checked!)
	 **/
	public static int getPixelARGB8(IImageLine line, int column) {
		if (line instanceof ImageLineInt) {
			if (((ImageLineInt) line).imgInfo.channels != 4)
				return getPixelRGB8(line, column);
			...
			if (((ImageLineByte) line).imgInfo.channels != 4)
				return getPixelRGB8(line, column);
	 ...
	 * 
	 * This only makes sense for RGB images, with bitdepth=8 (this is not
	 * checked!)
	 **/
